	...
	<cffunction name="GetStats" access="public" output="false" returntype="query" hint="return council statistics based on type">
		...
		<cfscript>
			...
			if (Len(arguments.orderby))
					qryStats	= OrderStats(qryStats, arguments.orderby);
			return 	qryStats;
		</cfscript>
  ...
  <!--- ----------------------------------------------------------------------------------------------------->
	...
	<cffunction name="OrderStats" access="public" returntype="query" output="false" hint="ReOrder Stats qry: pass in column name, to re-oder (cannot be done in SQL as dynamic cross tab)">
		...
		<cfscript>
			...
			var qry = objUtils.QueryOfQuery(variables.qryAttributes, 'datatypecode', "upper(Attribute) =  upper('#column#')");
			var datatype = qry.datatypecode; 
			
			//check if data type of column is text
			/*if(DataType eq 'TEXT')
			...
			qryStat=objUtils.QueryOfQuery(arguments.qryStat, '*', '0=0 ORDER BY #arguments.columnName# DESC');
				...
				//loop over qry
				for (i=1;i LTE qryStats.Recordcount; i=i+1){
				  //set array to hold values of column	
				  arr[i] =  qryStats.Evaluate(arguments.columnName)[i];
				  }
				//update qry tol hold new column
				QueryAddColumn(qryStats, 'PubSeq', datatype, arr);
				//reorder qry by new column
				qryStat=objUtils.QueryOfQuery(arguments.qryStat, '*', '0=0 ORDER BY pubseq DESC');
			}
			return qryStat;*/
		</cfscript>
			
		<cfif DataType eq 'TEXT' OR  arguments.columnName eq 'council'>
			<!---reorder qry by orgimnal column--->
			<cfset qryStat=objUtils.QueryOfQuery(arguments.qryStat, '*', '0=0 ORDER BY #arguments.columnName#')>
		
		<!---create new column casting it, and popolate with orginal's value--->
		<cfelse>
			<!--- <cfloop query="qryStats"> --->
				<!---set array to hold values of column --->
				<!---<cfset arr[currentrow] = Evaluate(arguments.columnName)>
			</cfloop> --->
			<!---  update qry tol hold new column --->
			<cfset QueryAddColumn(qryStats, 'PubSeq', 'INTEGER', arr)>
			<!---reorder qry using qryofqry--->
			<cfset qryStat=objUtils.QueryOfQuery(arguments.qryStat, '*', '0=0 ORDER BY #arguments.columnName# DESC')>
		</cfif>
	...
	<cffunction name="GetHome" access="public" returntype="struct" output="false" hint="return various qry's to be used on directory home page">
		<cfscript>
			...
			strReturn.StrCouncil = GetCouncilDetails(variables.strConfig.strVars.directory_org_sample_id);
